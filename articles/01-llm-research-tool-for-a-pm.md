---
title: "Designing an LLM-Powered Research Tool for a Hedge Fund PM"
date: 2026-07-01
type: article
tags: [system-design, llm, hedge-fund, agentic, rag, mcp, observability, audit, multi-tenant]
related: [hedgineer-sd-prep, 02-multi-tenant-llm-platform, 03-market-data-etl]
audience: [senior-engineer, staff-engineer, system-design-interview-prep]
estimated_read_time: 30 min deep read, 12 min skim
---

# Designing an LLM-Powered Research Tool for a Hedge Fund PM
...
The shape of this article: the question, the clarifying questions I ask, the architecture I draw, the flow I describe, the failure modes I name, and the close. Read it once end-to-end, then practice saying it out loud.

> Cross-link: this article leans on the system design vault's [RAG Pipeline](/notes/system-design/rag-pipeline-chunking-embeddings-retrieval-reranking/) and [Multi-Agent Orchestration](/notes/system-design/multi-agent-orchestration-planning-tool-use-memory-routing/) articles for the underlying mechanics. Read this article as a domain application of those primitives.

## The question

> "One of our clients is a $4B AUM long/short equity fund. Their PMs spend hours every morning doing pre-market prep — pulling positions, checking overnight moves, reading internal research notes, scanning news, looking at the FOMC calendar. They want an AI agent they can talk to.
>
...
Three things to notice before I draw anything. The interviewer is signaling they want to see:
...
1. **"Single tenant or multi-tenant?"** The first answer changes everything. If we're building for one fund, tenant isolation isn't load-bearing. If we're building a platform serving 5+ funds, isolation is the architecture. For Hedgineer specifically, the answer is multi-tenant — the JD says "client-facing." So I treat it as multi-tenant from the start.
...
3. **"What's the latency budget?"** I'll propose: first token < 1.5s, perceived completion < 5s for simple queries, end-to-end < 10s for agentic multi-step queries. If the interviewer pushes for faster, I defend the budgets.
4. **"What's the cost budget per query?"** I'll propose: $0.05 average per query across model mix. The economics matter — at 100 PMs × 10 queries/day × 250 trading days, that's $125K/year in LLM cost. The model choice drives this.
5. **"What does 'correct' mean to the PM?"** Does the PM verify the answer, or trust it? If they trust it, hallucination defense is critical. If they verify, the tool is a starting point and tolerance is higher. The JD says PMs "make decisions" — they trust it. The defense is critical.
...
I'll draw this as a layered system. Each layer is a separate concern with its own scaling, its own failure modes, its own observability.
...
Six layers. Each has a clear job. Let me walk through what each does and why.
...
This is the heart. The orchestrator is a LangGraph state machine with five nodes. Each node is an LLM call (or a deterministic step). The state is the partial plan + tool outputs so far.
...
The slow path: for an agentic query like "exposure to AAPL going into FOMC with relevant research", the intent classifier emits `{tools: [get_fund_exposure(AAPL), get_price_history(AAPL, "1mo"), search_research_notes("AAPL FOMC supply chain"), get_fomc_calendar]}`. We go to nodes 2-4.

**Node 2: Tool selection.** This is where LangGraph really shines. The orchestrator emits tool calls in parallel — every independent tool fires concurrently. Sequential calls only when there's a dependency (e.g., get_portfolio_positions before calculate_pnl).
...
The output includes `as_of` and `is_delayed` — the data is honest about its own freshness. This is the defense against stale-data presentation.
...
Let me walk through a specific query end-to-end so you can defend the timing and the order of operations.
...
I'll name six, in priority order. Each one: what fails, what the PM sees, what the system does.
...
The part I missed in the mock. The interviewer's hidden question: *can this person actually ship to a hedge fund, or do they just draw diagrams?*
...
3. **Single-PM pilot (1-2 weeks).** One PM uses the agent for real prep. Kill switch is a Slack command (`/agent-stop`). Daily check-ins.
...
> Cross-link: this article answers the flagship question. For the multi-tenant platform question, see [Article 2: Multi-Tenant LLM Platform](./02-multi-tenant-llm-platform.md). For the market data ETL question, see [Article 3: Market Data ETL](./03-market-data-etl.md). The gaps identified in the mock interview are documented in [Learning Record 0001](../learning-records/0001-mock-interview-2026-07-01.md).
