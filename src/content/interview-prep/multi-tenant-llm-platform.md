---
title: "Designing a Multi-Tenant LLM Platform for Asset Managers"
date: 2026-07-01
type: article
tags: [system-design, llm, multi-tenant, prompt-injection, isolation, audit, cost-attribution, interview-prep]
related: [llm-research-tool-for-an-asset-manager, market-data-etl, mock-interview-playbook]
difficulty: Staff
estimated_reading_time: 25
description: "The platform question that decides between Senior and Staff. Tenant isolation at every layer, prompt injection as a data exfiltration vector, per-tenant cost attribution, and the compliance officer UI as a second product surface."
---

# Designing a Multi-Tenant LLM Platform for Asset Managers
...
The question is not "design the PM-facing tool" — that's [the flagship article](/notes/interview-prep/llm-research-tool-for-an-asset-manager/). The question is "you're building this for 5 funds, not 1. How does the platform change?"
...
> Cross-link: this article assumes the architecture from the flagship. Read that first for the orchestrator, tool layer, and ingestion pipeline. This article goes deep on what changes when that design becomes a platform.
...
Same drill as the flagship. Five minutes of clarification.
...
The single-tenant design has six layers. The platform version adds three more concerns:
...
The single-tenant design becomes a platform by **hardening each layer to be tenant-aware and adding the compliance officer UI as a second product surface.** Walk each new concern.

### Per-tenant identity and access
...
**Critic agent (extended):** The critic not only checks hallucination (flagship) but also checks tenant isolation:
...
This is where most candidates under-deliver. Be specific.

| Storage | Isolation model | Why |
|---|---|---|
| **Postgres** (positions, users, audit log) | Row-level security (RLS) with `tenant_id` as a partition key. Every query has a `WHERE tenant_id = current_setting('app.tenant_id')` predicate. The DB enforces it, not the application. | RLS is the simplest model that scales. Per-tenant schema is more isolated but harder to maintain. Per-tenant DB is most isolated but operationally expensive. |
| **ClickHouse** (market data, time-series) | Per-tenant database. Each fund has its own ClickHouse DB. Cross-tenant queries are physically impossible. | ClickHouse is column-store, RLS is awkward. Per-tenant DB is cleaner. |
| **Milvus** (vector index for research notes) | Per-tenant index. Each fund has its own collection. Cross-tenant similarity is physically impossible. | The most underrated. A shared index could allow embedding-based cross-tenant retrieval. |
| **Blob storage (S3) — full documents** | Per-tenant prefix (`s3://{bucket}/{tenant_id}/...`). IAM policies enforce. | Standard pattern. |

**Embedding model versioning (cross-cutting):** Every embedding stores `(doc_id, chunk_text, embedding_vector, model_version)`. The model version is non-negotiable. When the model changes, new indexes are built and the migration is dual-index → atomic swap.
...
- 7-year retention
- Per-tenant namespace (`s3://{bucket}-audit/{tenant_id}/...`)
...
Same as the flagship, but with tenant onboarding as a first-class concern.

1. **Platform build (3-4 months).** Single-tenant architecture. Hardened for multi-tenant: tenant auth, RLS, per-tenant indexes, output classifier, compliance officer UI.
