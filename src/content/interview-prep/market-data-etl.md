---
title: "Designing the Market Data ETL Layer for a Finance Platform"
date: 2026-07-01
type: article
tags: [system-design, etl, market-data, olap, clickhouse, idempotent, lineage, staleness, interview-prep]
related: [llm-research-tool-for-an-asset-manager, multi-tenant-llm-platform, mock-interview-playbook]
difficulty: Senior
estimated_reading_time: 22
description: "The data layer question. Idempotency under failure, late-arriving data, schema evolution, OLAP vs OLTP, lineage for compliance, and the rule that stale data must be surfaced — not hidden."
---

# Designing the Market Data ETL Layer for a Finance Platform

The data layer question. If the interviewer has gone deep on the orchestrator and the platform, this is the third leg — the part that decides whether the system is actually production-grade or just a demo.
...
The question tests:
- **Idempotency.** Replay safety. Real-time market data feeds replay all the time. ETL must produce the same state on replay.
- **Late-arriving data.** Real feeds are messy. A tick for 13:24 ET arrives at 13:31 ET. The system has to handle it.
- **Schema evolution.** Vendors change formats. The system has to survive that without downtime.
...
3. **"What's the source mix?"** Market data vendor (refinitiv, polygon, IEX) for ticks. PDF uploads for research notes. Direct DB sync from the fund's PMS for positions. RSS or vendor push for news.
4. **"What's the data volume?"** Tick data: ~100M ticks/day for a mid-sized long/short equity fund. Research notes: ~50 PDFs/day, average 20 pages each. Positions: ~10K records, updated intraday.
...
│                          SOURCES                                      │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌────────────┐      │
│  │ Vendor     │  │ PDF upload │  │ PMS sync   │  │ News feed  │      │
...
│  │    • Vendor: WebSocket → normalized JSON                     │   │
...
│  ├──────────────────────────────────────────────────────────────┤   │
│  │ Tool layer (from the flagship article)                       │   │
│  │  • get_current_price, get_price_history, etc.                │   │
│  └──────────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────────┘
```

Five layers. Each does one job.

### Sources

Four source types, four adapter patterns:

1. **Market data vendor (refinitiv, polygon, IEX).** Real-time WebSocket or TCP feed. Vendor-specific protocol. The adapter normalizes to a common schema: `{symbol, price, volume, as_of, source}`. The adapter handles vendor outages, replays, and gap detection.
...
**Embedding generation:** For research chunks and news articles, the worker calls the embedding API. The embedding is stored alongside the chunk in Milvus, keyed by `(tenant_id, doc_id, chunk_id, model_version)`. The model version is non-negotiable.
...
This is the same pattern as the flagship, but at the storage layer.
...
3. **Tool layer.** The deterministic tools from the flagship:
...
**Defense:**
1. **Schema registry.** All schemas are versioned in a registry. The adapter validates against the current schema version.
...
Same as the flagship, but with the data layer as a first-class concern.
...
> Cross-link: the data layer feeds the orchestrator from the [flagship article](/notes/interview-prep/llm-research-tool-for-an-asset-manager/) and is partitioned per-tenant as described in the [Multi-Tenant LLM Platform](/notes/interview-prep/multi-tenant-llm-platform/) article. The mock interview gaps that drove this article are in the [Mock Interview Playbook](/notes/interview-prep/mock-interview-playbook/).
